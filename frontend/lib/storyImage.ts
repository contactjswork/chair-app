...
// ── Story « fauteuil à louer » — la pub de l'annonce (CHAIR BUSINESS) ──

export interface FauteuilStoryOptions {
  title: string;
  salonName: string;
  city?: string | null;
  /** Meilleur prix disponible, déjà formaté : « 45 €/jour », « 900 €/mois ». */
  priceLabel?: string | null;
  /** Première photo de l'annonce (Cloudinary) — fond sombre CHAIR sinon. */
  photoUrl?: string | null;
  /** Slug public — affiché comme getchair.app/fauteuil/{slug}. */
  slug?: string | null;
}

/**
 * Le visuel 1080×1920 qu'un gérant poste en story Instagram/Facebook ou dans
 * un groupe pro pour faire connaître son fauteuil libre — même famille
 * graphique que les stories coiffeur (fond noir CHAIR, wordmark, pilule lien).
 */
export async function genererStoryFauteuil(opts: FauteuilStoryOptions): Promise<Blob> {
  const photo = opts.photoUrl ? await chargerImage(opts.photoUrl).catch(() => null) : null;
  ...
  ctx.font = `700 34px -apple-system, 'Helvetica Neue', Arial, sans-serif`;
  ctx.fillStyle = 'rgba(255,255,255,0.5)';
  ctx.fillText('F A U T E U I L   À   L O U E R', W / 2, 310);

  // La photo de l'espace, si l'annonce en a une.
  let y: number;
  if (photo) {
    dessinerCover(ctx, photo, 80, 380, W - 160, 900, 40);
    y = 1400;
  } else {
    y = 720;
  }

  // Titre de l'annonce, salon, ville.
  ctx.font = `900 66px -apple-system, 'Helvetica Neue', Arial, sans-serif`;
  ctx.fillStyle = '#ffffff';
  ctx.fillText(opts.title, W / 2, y, W - 160);
  y += 70;

  ctx.font = `600 40px -apple-system, 'Helvetica Neue', Arial, sans-serif`;
  ctx.fillStyle = 'rgba(255,255,255,0.75)';
  ctx.fillText(opts.salonName + (opts.city ? ` · ${opts.city}` : ''), W / 2, y, W - 160);
  y += 90;

  if (opts.priceLabel) {
    ctx.font = `900 84px -apple-system, 'Helvetica Neue', Arial, sans-serif`;
    ctx.fillStyle = '#ffffff';
    ctx.fillText(opts.priceLabel, W / 2, y);
    y += 76;
  }

  ctx.font = `700 34px -apple-system, 'Helvetica Neue', Arial, sans-serif`;
  ctx.fillStyle = 'rgba(255,255,255,0.6)';
  ctx.fillText('Coiffeurs indépendants — postulez sur CHAIR', W / 2, photo ? H - 260 : y + 40);

  if (opts.slug) {
    const texte = `getchair.app/fauteuil/${opts.slug}`;
    ctx.font = `700 32px -apple-system, 'Helvetica Neue', Arial, sans-serif`;
    const l = ctx.measureText(texte).width;
    const cy = H - 150;
    ctx.fillStyle = 'rgba(255,255,255,0.1)';
