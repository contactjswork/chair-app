...
import {
  Building2, LayoutGrid, MapPin, Navigation, RotateCcw, Sparkles, Star, TrendingUp, User, X,
} from 'lucide-react';
...
function Section({ title, children, hint }: { title: string; children: React.ReactNode; hint?: string }) {
  return (
    <div>
      <p className="text-[10px] font-bold tracking-[0.25em] uppercase text-neutral-400 mb-4">{title}</p>
      {children}
...
function Chip({ active, onClick, icon, children }: { active: boolean; onClick: () => void; icon?: React.ReactNode; children: React.ReactNode }) {
  return (
    <button
      onClick={onClick}
      className={`flex items-center gap-1.5 text-[13px] font-semibold px-4 py-2.5 rounded-2xl border transition-all ${
        active
    ...
    <div className="fixed inset-0 z-[85] flex flex-col justify-end md:items-center md:justify-center">
      ...
      <div className="relative flex flex-col bg-white rounded-t-3xl md:rounded-3xl shadow-2xl max-h-[92vh] md:max-h-[85vh] md:w-full md:max-w-lg overflow-hidden">
        ...
        <div className="flex items-center justify-between px-5 py-3 border-b border-neutral-100 flex-shrink-0">
          <h2 className="text-[17px] font-bold text-neutral-900">Filtres</h2>
          <button onClick={onClose} aria-label="Fermer" className="w-8 h-8 rounded-full bg-neutral-100 flex items-center justify-center">
        ...
        {/* Actions sticky */}
        <div className="flex-shrink-0 border-t border-neutral-100 px-5 py-4 pb-safe flex gap-3 bg-white">
          <button
            onClick={onReset}
