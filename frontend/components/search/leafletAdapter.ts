 ...
 *  LEVEL_RING (lib/chairLevel.ts), en hex brut car ce HTML est injecté hors
 *  du pipeline Tailwind (divIcon = chaîne de caractères, pas du JSX). */
 ...
 *  niveau CHAIR (l'anneau de progression déjà utilisé partout ailleurs dans
 *  l'app), avec la note en pastille superposée. Photo si dispo, initiale sinon. */
...
/** Marqueur fauteuil à louer — pastille prix façon Airbnb, jamais utilisée
 *  sur la carte de recherche client (voir ChairSearchMap, un composant à part). */
...
function markerHtml(m: MapMarkerData, selected: boolean): string {
 ...
 *  autour du point — décalage purement visuel, jamais persisté. */
