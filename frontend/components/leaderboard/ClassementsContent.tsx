...
// ── Contenu du classement, partagé entre CHAIR (/app/classements, dans
// AppShell) et CHAIR PRO (/pro/classements, dans le shell pro) — retour de
// Julien : cliquer "Classement" depuis CHAIR PRO ouvrait l'interface CHAIR
// (mauvaise appli sans en changer). Chaque appli garde son propre wrapper
// de mise en page ; seul `linkTarget` change entre les deux : les fiches
// coiffeur restent une expérience CHAIR (client), donc s'ouvrent dans un
// nouvel onglet quand ce composant est monté côté pro (même pattern déjà
// utilisé pour les aperçus "Mon profil"/"Profil public" ailleurs dans
// CHAIR PRO), plutôt que de remplacer le shell pro en place. ──────────────
...
function PodiumCard({ entry, size, linkTarget }: { entry: DisplayEntry; size: 'lg' | 'sm'; linkTarget: '_self' | '_blank' }) {
...
function RankRow({ entry, isMe, linkTarget }: { entry: DisplayEntry; isMe?: boolean; linkTarget: '_self' | '_blank' }) {
  const avatar = resolveMediaUrl(entry.avatar);
  const initial = (entry.name ?? '?').charAt(0).toUpperCase();
...
interface Props {
  /** '_blank' (CHAIR PRO) : les fiches coiffeur — expérience CHAIR client —
   *  s'ouvrent dans un nouvel onglet plutôt que de remplacer le shell pro en
   *  place. '_self' (CHAIR) : navigation normale dans l'appli. */
  linkTarget?: '_self' | '_blank';
  /** Padding bas additionnel — AppShell (CHAIR) gère sa propre BottomNav en
   *  fixed, contrairement au shell pro (déjà géré par app/pro/layout.tsx). */
  bottomPadding?: string;
}

export default function ClassementsContent({ linkTarget = '_self', bottomPadding = 'pb-28 md:pb-8' }: Props) {
  const { user } = useAuth();
  ...
  return (
    <div className={`max-w-2xl mx-auto ${bottomPadding} overflow-x-hidden`}>
      {/* Header */}
        ...
        <>
          ...
          {podium.length > 0 && (
            <div className="px-4 mb-4 flex items-end gap-2">
              {podium[1] && <PodiumCard entry={podium[1]} size="sm" linkTarget={linkTarget} />}
              {podium[0] && <PodiumCard entry={podium[0]} size="lg" linkTarget={linkTarget} />}
              {podium[2] && <PodiumCard entry={podium[2]} size="sm" linkTarget={linkTarget} />}
            </div>
          )}
          ...
          {rest.length > 0 && (
            <div className="px-4 pt-2">
              <div className="space-y-2">
                {rest.map((entry) => (
                  <RankRow key={entry.id} entry={entry} isMe={entry.id === myProfileId} linkTarget={linkTarget} />
                ))}
              </div>
            </div>
          )}
