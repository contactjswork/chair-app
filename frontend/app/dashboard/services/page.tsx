...
function ServiceForm({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (isIndependent && (price === null || duration === null)) return;
    setLoading(true);
    try {
      await onSave({
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="text-xs text-neutral-500 mb-1 block">
            Prix (€){!isIndependent && <span className="text-neutral-400"> — facultatif</span>}
          </label>
          <input
            type="number"
            value={price ?? ''}
            ...
            min={0}
            step={0.5}
            placeholder={isIndependent ? '0' : 'Au salon'}
            className="w-full border border-neutral-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-neutral-400"
        ...
        <div>
          <label className="text-xs text-neutral-500 mb-1 block">
            Durée{!isIndependent && <span className="text-neutral-400"> — facultatif</span>}
          </label>
          {isIndependent ? (
            <select
              value={duration ?? 30}
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-3 mt-1">
                            {svc.price != null
                              ...
                              : <span className="text-xs text-neutral-400 italic">Tarifs au salon</span>
                            }
